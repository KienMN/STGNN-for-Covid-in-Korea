...
from stgraph_trainer.models.seq2seq import Encoder, Decoder, DecoderWithAttention
...
X_train, y_train, X_test, y_test, raw_train, raw_test, scaler = preprocess_data_for_seq2seq(df, SPLIT_DATE, TIME_STEPS)
...
for trial in range(TRIALS):
  encoder = Encoder(ENC_DENSE_UNITS, ENC_UNITS, BATCH_SIZE, dropout=DROP_RATE, recurrent_dropout=RECURRENT_DROP_RATE)
  # decoder = Decoder(1, DEC_UNITS, BATCH_SIZE, dropout=DROP_RATE, recurrent_dropout=RECURRENT_DROP_RATE)
  decoder = DecoderWithAttention(1, DEC_UNITS, BATCH_SIZE, dropout=DROP_RATE, recurrent_dropout=RECURRENT_DROP_RATE)
